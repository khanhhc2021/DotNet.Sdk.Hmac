...
namespace Cloud_Net_Sdk_Hmac_Lib.Signers
{
    ...
    public class SignerForAuthorizationHeader : SignerBase
    {
        ...
        /// </returns>
        /// 
        public AuthorizationModel ComputeSignature(IDictionary<string, string> headers,
            string path,
            ...
            string awsSecretKey)
        {
            AuthorizationModel mRet = new AuthorizationModel();
            ...
            var canonicalRequest = CanonicalizeRequest(EndpointUri,
                ...
                bodyHash);
            Debug.WriteLine("\nCanonicalRequest:\n{0}", canonicalRequest);
            mRet.CanonicalRequest = canonicalRequest;
            ...
            Debug.WriteLine("\nStringToSign:\n{0}", stringToSign);
            mRet.StringToSign = stringToSign.ToString();
            // compute the signing key
            var kha = KeyedHashAlgorithm.Create(HMACSHA256);
            var signingKey = ToHexString(DeriveSigningKey(HMACSHA256, awsSecretKey, path, dateStamp), true);
            Debug.WriteLine("\nSigning_key:{0}", signingKey);
            kha.Key = Encoding.UTF8.GetBytes(signingKey);
